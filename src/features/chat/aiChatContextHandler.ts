...
import { getOpenFilesList } from '../../core/workspace/editorUtils/getOpenFilesList';
...
export class AiChatContextHandler {
    ...
    private readonly recentFilesKey = 'recentlyOpenedFiles';
    ...
    private getLastFiveFiles(): Set<{ fileName: string, filePath: string, languageId: string }> {
        const storedFiles = this.context.workspaceState.get<{ fileName: string, filePath: string, languageId: string }[]>(this.recentFilesKey, []);
        return new Set(storedFiles.map(file => ({ ...file })));
    }

    private updateLastFiveFiles() {
        this.context.workspaceState.update(this.recentFilesKey, Array.from(this.recentlyOpenedFiles));
    }
