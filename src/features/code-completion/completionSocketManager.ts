...
import { versionConfig } from '../../core/config/versionConfig';
import {
...
export class CompletionSocketManager {
  ...
  private predictionHandleFunction(predictionReceived: any): void {
    ...
    if (this.tempUniqueIdentifier !== prediction.uniqueId) {
    ...
    this.currentSuggestionId = prediction.uniqueId;
    this.suggestion = this.extractSuggestion(prediction.message);
    this.socketListSuggestion = prediction.messageList;
